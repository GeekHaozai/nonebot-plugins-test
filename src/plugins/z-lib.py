...
async def send_msg(bot: Bot, event: MessageEvent, msg: Message = CommandArg()):
    ...
    await bot.send(event=event,group_id=group, message="测试z-lib的引用回复", reply_message=True,at_sender=True)
    try:
        # 判断消息类型
        if msg_from == "group":
            await bot.send_group_forward_msg(group_id=group, messages=msgList)
            await Bot.send(bot=bot,group_id=group, message="测试z-lib的引用回复",reply_message=True)
        else:
